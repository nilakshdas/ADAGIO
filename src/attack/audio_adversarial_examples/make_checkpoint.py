...
import binascii
if binascii.crc32(open(os.path.join(DEEPSPEECH_LIB_DIR, "DeepSpeech.py"), "rb").read()) != 1142193310:
    print("-"*80)
    print("-"*80)
    print("WARNING")
    print("It looks like you have the wrong version of DeepSpeech installed.")
...
graph_def = GraphDef()
loaded = graph_def.ParseFromString(open(os.path.join(DEEPSPEECH_MODELS_DIR, "output_graph.pb"), "rb").read())
...
with tf.Graph().as_default() as graph:
    ...
    with tf.Session(graph=graph) as sess:
        ...
        tf.app.flags.FLAGS.alphabet_config_path = os.path.join(DEEPSPEECH_LIB_DIR, "data/alphabet.txt")
        ...
        saver.save(sess, os.path.join(DEEPSPEECH_MODELS_DIR, "session_dump"))
